...
# With hourly scan CSV saving + download page
...
def format_dhan_portfolio_table():
    ...
    if not h_df.empty:
        ...
        portfolio = h_df[
            ["_name", "_qty", "_avg", "_total_cost", "_cmp", "_total_price", "_pnl"]
        ].rename(
            columns={
                "_name": "Stock",
                "_qty": "Quantity",
...
class TechnicalAnalysis:
    ...
    def ema_crossover(df):
        ...
        if safe_scalar(ema9.iloc[-1]) > safe_scalar(ema21.iloc[-1]) and safe_scalar(ema9.iloc[-2]) <= safe_scalar(
            ema21.iloc[-2]
        ):
...
def run_analysis() -> pd.DataFrame:
    ...
    for p in ["BTST", "Intraday", "Weekly", "Monthly"]:
        recs = analyze_multiple_stocks(STOCK_UNIVERSE, p, max_results=20)
        st.session_state["recommendations"][p] = recs
        for r in recs:
            r2 = dict(r)
            r2["period"] = p
            all_rows.append(r2)
...
def market_hours_window(dt_: datetime) -> bool:
    # legacy: 9:10–15:40
    start = dt_.replace(hour=9, minute=10, second=0, microsecond=0)
    end = dt_.replace(hour=15, minute=40, second=0, microsecond=0)
    return start <= dt_ <= end

def is_market_time(now: Optional[datetime] = None) -> bool:
    if now is None:
        now = datetime.now(IST)
    if now.weekday() > 4:
        return False
...
NOTIFY_SLOTS = ["09:30", "13:30", "15:00"]  # IST
...
def run_scan_and_save_csv_if_due(now: Optional[datetime] = None):
    if now is None:
        now = datetime.now(IST)
    ...
    if last is None:
        ...
        try:
            if (now - last).total_seconds() >= 60 * 60:  # 1 hour
                should_run = True
        except Exception:
            should_run = True
    if should_run:
        ...
        st.caption(f"🕒 Hourly auto-scan executed at {now.strftime('%H:%M:%S')} IST")
    handle_scheduled_notifications(now)

def auto_scan_if_due():
    now = datetime.now(IST)
    run_scan_and_save_csv_if_due(now)
...
def render_reco_cards(recs: List[Dict], label: str):
    ...
    for _, rec in df.iterrows():
        ...
        st.markdown(
            f"<div class='value'>💰 CMP: ₹{cmp_:.2f}  | 🎯 Target: ₹{tgt_display}</div>",
            unsafe_allow_html=True,
        )
...
def analyze_dhan_portfolio_recommendations():
    ...
    st.markdown(
        "For each stock below: recommendation (Buy/Hold/Sell), target price, how far CMP is from recommended price, "
        "present P&L, and upcoming dividend (if any)."
    )
    st.write("")
    ...
    for _, row in df_port.iterrows():
        ...
        try:
            ...
            div_amt = info.get("lastDividendValue") or info.get("dividendRate") or info.get(
        ...
        st.markdown("<div class='metric-card'>", unsafe_allow_html=True)
        st.markdown(
            f"<h3>{stock_name} • {rec} • {timeframe_label}</h3>",
            unsafe_allow_html=True,
        )
        st.markdown(
            f"<div class='value'>💰 CMP: ₹{cmp_val:.2f}  | 🎯 Primary Target: {tgt_display}</div>",
            unsafe_allow_html=True,
        )
        ...
        if upcoming_div_date and upcoming_div_amount:
            div_lines = (
                f"📅 Upcoming ex/div date: {upcoming_div_date.isoformat()} • "
                f"Per-share: ₹{upcoming_div_amount:.2f}"
            )
            if total_upcoming_div_payout is not None:
                div_lines += (
                    f" • Expected payout: ₹{total_upcoming_div_payout:,.2f} "
                    f"(for {int(qty)} shares)"
                )
        ...
        if best_summary:
            ...
            st.markdown(
                f"<div class='sub'>🧠 {timeframe_label} scanner: {strength} • Score: {score} • {reason}</div>",
                unsafe_allow_html=True,
            )

        st.markdown("</div>", unsafe_allow_html=True)

def page_hourly_scans():
    st.subheader("🕒 Hourly Scan Downloads")
    if not SCANS_DIR.exists():
        st.info("No scan files available yet.")
        return
    csv_files = sorted(glob.glob(str(SCANS_DIR / "scan_*.csv")), reverse=True)
    if not csv_files:
        st.info("No scan files available yet.")
        return
...
def main():
    ...
    if st.session_state["last_analysis_time"]:
        st.caption(
            f"🕒 Last Full Scan: {st.session_state['last_analysis_time'].strftime('%d-%m-%Y %I:%M %p')}"
        )
    st.info(
        "On mobile you can view Top 20 stocks. For other views (BTST, Intraday, Weekly, Monthly, "
        "Groww, Dhan, Hourly Scans, Configuration), please open this dashboard on a laptop or desktop.",
        icon="📱",
    )
    st.markdown("---")
    ...
    if page == "🔥 Top Stocks":
        ...
        st.markdown(
            "Upload your Groww holdings file (CSV/XLS/XLSX) to get advanced analytics with all values in ₹ INR."
        )
        st.code(
        ...
        if uploaded is not None:
            ...
            for i, row in df.iterrows():
                ...
                div_y, div_r, cagr = fetch_dividend_and_cagr(
                    stock_name,
                    str(row[cols["isin"]]),
                    cmp_ps,
                )
            ...
            st.markdown(
                snap_df.to_html(classes="dark-table", index=False, escape=False),
                unsafe_allow_html=True,
            )
            ...
            st.markdown(
                "*Based on weighted CAGR and total yearly dividend. All values shown in Lakhs.*"
            )
            ...
            st.markdown(
                proj_df.to_html(classes="dark-table", index=False, escape=False),
                unsafe_allow_html=True,
            )
            ...
            df_reco = df[base_cols + ["Strength"]].rename(
                columns={
        ...
        if dhan_store:
            st.session_state["dhan_client_id"] = dhan_store.get(
                "client_id", st.session_state["dhan_client_id"]
            )

        dhan_enable = st.checkbox(
            "Enable Dhan",
            value=st.session_state.get("dhan_enabled", False),
        )
        ...
        if dhan_enable:
            ...
            st.markdown(
                cfg_df.to_html(classes="dark-table", index=False, escape=False),
                unsafe_allow_html=True,
            )
            ...
            if df_port is None or df_port.empty:
                ...
                with c2:
                    st.markdown(
                        ...
                        f"<div class='value'>₹{total_pnl:,.2f}</div></div>",
                        unsafe_allow_html=True,
                    )
        else:
        ...
        with st.expander("📨 Telegram P&L Notifications", expanded=False):
            ...
            if tg_store:
                st.session_state["telegram_bot_token"] = tg_store.get(
                    "bot_token", st.session_state["telegram_bot_token"]
                )
                st.session_state["telegram_chat_id"] = tg_store.get(
            ...
            tg_rows = [
                ...
                {
                    "Field": "Notifications",
                    "Value": "Enabled" if notify_toggle else "Disabled",
                },
            ]
            tg_df = pd.DataFrame(tg_rows)
            st.markdown(
                tg_df.to_html(classes="dark-table", index=False, escape=False),
                unsafe_allow_html=True,
            )
            ...
            with c2:
                if st.button(
                    ...
                    tg_resp = (
                        ...
                        else {"info": "Telegram not configured"}
                    )
